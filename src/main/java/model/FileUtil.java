...
public class FileUtil {
    ...
    // Overloaded method to write an Order object directly to a file
    ...
    // Read all delivered orders from deliveredOrders.txt
    ...
    // Write all orders to orders.txt
    ...
    // Write all delivered orders to deliveredOrders.txt
                    ...
                    if (parts.length == 2) {
                        ...
                        switch (key) {
                            ...
                            case "phoneNumber":
                                ...
                                break;
                            case "address":
                                // Remove quotes if present, as in sample: address="Sampatha"
                                user.setAddress(value.startsWith("\"") && value.endsWith("\"") ? value.substring(1, value.length() - 1) : value);
                                break;
                        }
                    }
                }
    ...
    public static void writeUsers(String filePath, List<User> users) throws IOException {
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(file, false))) {
            for (User user : users) {
                writer.write("--- User Start: " + (user.getUserNumber() != null ? user.getUserNumber() : "") + " ---\n");
                ...
                // Add quotes around address to match sample format: address="Sampatha"
    ...
    // Read an admin by email from admins.txt
