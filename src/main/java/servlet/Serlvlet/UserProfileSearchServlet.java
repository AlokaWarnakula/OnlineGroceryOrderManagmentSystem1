...
public class UserProfileSearchServlet extends HttpServlet {
    //File paths use to store the data of orders placed
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        //get the current user's information from the session to know who is logged in
        User loggedInUser = (User) session.getAttribute("user");

        if (loggedInUser == null) {
            //redirect to the login page if no user is found as logged-in
            response.sendRedirect(request.getContextPath() + "/userLogin/login.jsp");
            return;
        }
//Extract the number of the user and the tab type from request parameters
        ...
        //List for holding the filtered orders
        List<FileUtil.Order> orders;
        if ("Delivered".equalsIgnoreCase(tab)) {
            //Read the delivered orders from file if tab is "Delivered"
            orders = FileUtil.readAllDeliveredOrders(DELIVERED_ORDERS_FILE);
        } else {
//if tab is not "Delivered" read all orders from a different file
            orders = FileUtil.readAllOrders(ORDERS_FILE);
        }

        // Filter orders by user number
        orders = orders.stream()
                .filter(order -> userNumber.equals(order.getUserNumber()))
                .collect(Collectors.toList());

        // Apply search filter if searchQuery is provided and if it's not empty
        if (searchQuery != null && !searchQuery.trim().isEmpty()) {
            String query = searchQuery.trim().toLowerCase();
            //Filter orders which the order numbers contain the search query
            orders = orders.stream()
                    .filter(order -> order.getOrderNumber().toLowerCase().contains(query))
                    .collect(Collectors.toList());
        }

        // Sort orders based on the selected tab
        if ("Delivered".equalsIgnoreCase(tab)) {
